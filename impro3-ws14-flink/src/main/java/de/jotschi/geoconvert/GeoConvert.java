...
public class GeoConvert {
     ...
     * 
     * Reference: Hoffmann-Wellenhof, B., Lichtenegger, H., and Collins, J., GPS: Theory and
     * Practice, 3rd ed. New York: Springer-Verlag Wien, 1994.
     * 
     * Inputs: phi - Latitude of the point, in radians.
     * 
     * Globals: sm_a - Ellipsoid model major axis. sm_b - Ellipsoid model minor axis.
     * 
    ...
    public static double ArcLengthOfMeridian(double phi) {
        ...
        result =
                alpha
                        * (phi + (beta * Math.sin(2.0 * phi)) + (gamma * Math.sin(4.0 * phi)) + (delta * Math.sin(6.0 * phi)) + (epsilon * Math.sin(8.0 * phi)));
     ...
     * 
     * Returns: The central meridian for the given UTM zone, in radians, or zero if the UTM zone
     * parameter is outside the range [1,60]. Range of the central meridian is the radian equivalent
     * of [-177,+177].
     */
     ...
     * FootpointLatitude
     * 
     * Computes the footpoint latitude for use in converting transverse Mercator coordinates to
     ...
     * MapLatLonToXY
     * 
     * Converts a latitude/longitude pair to x and y coordinates in the Transverse Mercator
     * projection. Note that Transverse Mercator is not the same as UTM; a scale factor is required
     * to convert between them.
     * 
     * Reference: Hoffmann-Wellenhof, B., Lichtenegger, H., and Collins, J., GPS: Theory and
     * Practice, 3rd ed. New York: Springer-Verlag Wien, 1994.
     * 
     * Inputs: phi - Latitude of the point, in radians. lambda - Longitude of the point, in radians.
     * lambda0 - Longitude of the central meridian to be used, in radians.
     * 
     * Outputs: xy - A 2-element array containing the x and y coordinates of the computed point.
     * 
    ...
    public static void MapLatLonToXY(double phi, double lambda, double lambda0, double[] xy) {
        ...
        /*
         * Precalculate coefficients for ln in the equations below so a normal human being can read
         * the expressions for easting and northing -- l1 and l2 have coefficients of 1.0
         */
        l3coef = 1.0 - t2 + nu2;
        ...
        /* Calculate easting (x) */
        xy[0] =
                N * Math.cos(phi) * l + (N / 6.0 * Math.pow(Math.cos(phi), 3.0) * l3coef * Math.pow(l, 3.0))
                        + (N / 120.0 * Math.pow(Math.cos(phi), 5.0) * l5coef * Math.pow(l, 5.0))
                        + (N / 5040.0 * Math.pow(Math.cos(phi), 7.0) * l7coef * Math.pow(l, 7.0));

        /* Calculate northing (y) */
        xy[1] =
                ArcLengthOfMeridian(phi) + (t / 2.0 * N * Math.pow(Math.cos(phi), 2.0) * Math.pow(l, 2.0))
     ...
     * MapXYToLatLon
     * 
     * Converts x and y coordinates in the Transverse Mercator projection to a latitude/longitude
     * pair. Note that Transverse Mercator is not the same as UTM; a scale factor is required to
     ...
     * Returns: The function does not return a value.
     * 
     * Remarks: The local variables Nf, nuf2, tf, and tf2 serve the same purpose as N, nu2, t, and
     * t2 in MapLatLonToXY, but they are computed with respect to the footpoint latitude phif.
     * 
    ...
    public static void MapXYToLatLon(double x, double y, double lambda0, double[] philambda) {
        ...
        /*
         * Precalculate fractional coefficients for xn in the equations below to simplify the
         * expressions for latitude and longitude.
         */
        x1frac = 1.0 / (Nfpow * cf);
        ...
        /*
         * Precalculate polynomial coefficients for xn. -- x1 does not have a polynomial
         * coefficient.
         */
        x2poly = -1.0 - nuf2;
        ...
        /* Calculate latitude */
        philambda[0] =
                phif + x2frac * x2poly * (x * x) + x4frac * x4poly * Math.pow(x, 4.0) + x6frac * x6poly * Math.pow(x, 6.0) + x8frac * x8poly
                        * Math.pow(x, 8.0);

        /* Calculate longitude */
        philambda[1] =
                lambda0 + x1frac * x + x3frac * x3poly * Math.pow(x, 3.0) + x5frac * x5poly * Math.pow(x, 5.0) + x7frac * x7poly * Math.pow(x, 7.0);

    }

    public static double[] getBounds(double[] xy1, double[] xy2) throws Exception {
        ...
        double[] utmBounds = {Math.abs(utm1[0] - utm2[0]), Math.abs(utm1[1] - utm2[1])};

        return utmBounds;
    }

    public static int[] getPos(double lon, double lat, double[] xy1, double[] xy2, int height, int width) throws Exception {
     ...
     * LatLonToUTMXY
     * 
     * Converts a latitude/longitude pair to x and y coordinates in the Universal Transverse
     * Mercator projection.
     * 
     * Inputs: lat - Latitude of the point, in radians. lon - Longitude of the point, in radians.
     * zone - UTM zone to be used for calculating values for x and y. If zone is less than 1 or
     * greater than 60, the routine will determine the appropriate zone from the value of lon.
     * 
     ...
     * UTMXYToLatLon
     * 
     * Converts x and y coordinates in the Universal Transverse Mercator projection to a
     * latitude/longitude pair.
     * 
     * Inputs: x - The easting of the point, in meters. y - The northing of the point, in meters.
     * zone - The UTM zone in which the point lies. southhemi - True if the point is in the southern
     * hemisphere; false otherwise.
     * 
     * Outputs: latlon - A 2-element array containing the latitude and longitude of the point, in
     * radians.
     * 
     * Returns: The function does not return a value.
     */
    ...
    public static double[] toUtm(double lon, double lat) throws Exception {
        ...
        if ((lon < -180.0) || (180.0 <= lon)) {
            throw new Exception("The longitude you entered is out of range.  " + "Please enter a number in the range [-180, 180).");

        }

        if ((lat < -90.0) || (90.0 < lat)) {
            throw new Exception("The latitude you entered is out of range.  " + "Please enter a number in the range [-90, 90].");
    ...
    public static String toMGRS(double lon, double lat) throws Exception {
        ...
        String[][] eastings =
                { {"S", "T", "U", "V", "W", "X", "Y", "Z"}, {"A", "B", "C", "D", "E", "F", "G", "H"}, {"J", "K", "L", "M", "N", "P", "Q", "R"}};

        String[] northings = {"A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "Q", "R", "S", "T", "U", "V"};
    ...
    public static String getZoneBand(double lat) throws Exception {
        ...
        String[] bands = {"C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "Q", "R", "S", "T", "U", "V", "W"};
