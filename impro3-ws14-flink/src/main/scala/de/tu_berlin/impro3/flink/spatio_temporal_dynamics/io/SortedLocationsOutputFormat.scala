...
class SortedLocationsOutputFormat[T] extends OutputFormat[T] {
  ...
  override def configure(parameters: Configuration): Unit = try {
    ...
    table = conn.getTable(TableName.valueOf("SortedLocations"))
  ...
  override def writeRecord(record: T): Unit = record match {
    case (loc: String, (occ: Int, (lat: Double, lng: Double))) =>
      val put = new Put(Bytes.toBytes(occ))
      put.addColumn(Bytes.toBytes("location"), Bytes.toBytes(loc +":latitude"), Bytes.toBytes(lat))
      put.addColumn(Bytes.toBytes("location"), Bytes.toBytes(loc +":longitude"), Bytes.toBytes(lng))
      table.put(put)
