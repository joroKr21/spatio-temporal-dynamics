...
import de.tu_berlin.impro3.flink.spatio_temporal_dynamics.io.HBaseOutputFormat._
...
object SpatioTemporalDynamicsPerCity extends scala.App {
  val description = """
  ...
  val byCity = clusterByCity(tags).filterTotalSize(locSize)
  val byText = (tags
    .groupBy { _.text }
    .reduceGroup { tags =>
      val seq = tags.toStream
      seq.head.text -> seq.groupBy { _.zone }
    }: Cluster).filterTotalSize(tagSize)

  byText.metric { set =>
    (Occurrences(set), Focus(set), Entropy(set), Spread(set), Lifespan(set))
  }.output(HBaseOutputFormat("HashtagsPerCity", "Cities") {
  ...
  }.output(HBaseOutputFormat("HashtagsPerCity") {
  ...
  }.output(HBaseOutputFormat("SortedCities") {
  ...
  byText.metric { set =>
  ...
  }.output(HBaseOutputFormat("SortedHashtagsPerCity",
                             "HashtagDistributionPerCity") {
    case (tables, (tag, (midpoints, distribution))) =>
      ...
      tables("HashtagDistributionPerCity").put(hashTags)
  })
